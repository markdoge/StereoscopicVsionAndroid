...
public class StdCamera extends AppCompatActivity {
    ...
    private final int cametacount= Camera.getNumberOfCameras();
    ...
    StdCamera(SurfaceView temp){
        ...
        SurfaceHolder camViewHolder = temp.getHolder();
        camViewHolder.setFormat(PixelFormat.TRANSPARENT);
        //通过SurfaceView显示取景画面
        //开始预览
        //设置是否预览参数为真
        //正在预览
        SurfaceHolder.Callback camSurfaceCallback = new SurfaceHolder.Callback() {
            @Override
