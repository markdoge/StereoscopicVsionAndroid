...
public class TestScroller extends View{
    ...
    private void initAttrs(AttributeSet attrs) {
        TintTypedArray tta = TintTypedArray.obtainStyledAttributes(getContext(), attrs,
                R.styleable.TestScroller);
        //两种字体颜色和字体大小
        seeSize = tta.getInteger(R.styleable.TestScroller_TestScrollerSeeSize, 5);
        selectedTextSize = tta.getFloat(R.styleable.TestScroller_TestScrollerSelectedTextSize, 50);
        selectedColor = tta.getColor(R.styleable.TestScroller_TestScrollerSelectedTextColor,
                context.getResources().getColor(android.R.color.black));
        textSize = tta.getFloat(R.styleable.TestScroller_TestScrollerTextSize, 40);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (n >= 0 && n <= strings.size() - 1) {//加个保护；防止越界
            ...
            canvas.drawText(strings.get(n), getWidth() / 2 - centerTextWidth / 2 + anOffset,
                    getHeight() / 2 + centerTextHeight / 2, selectedPaint);//绘制被选中文字，注意点是y坐标
            ...
            for (int i = 0; i < strings.size(); i++) {//遍历strings，把每个地方都绘制出来，
                ...
                if (i != n)
                    canvas.drawText(strings.get(i), (i - n) * anInt + getWidth() / 2 - textWidth / 2 + anOffset,
                            getHeight() / 2 + textHeight / 2, textPaint);//画出每组文字
